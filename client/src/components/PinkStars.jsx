...
export default function PinkStars({ emphasized = false }) {
  ...
  useEffect(() => {
    ...
    if (!el) return;
    el.innerHTML = '';
    const count = emphasized ? 124 : 96;
    for (let i = 0; i < count; i++) {
      const s = document.createElement('div');
      const large = Math.random() < (emphasized ? 0.34 : 0.3);
      const hue = emphasized
        ? (Math.random() < 0.5 ? '255,238,244' : Math.random() < 0.82 ? '255,214,226' : '232,178,192')
        : (Math.random() < 0.55 ? '255,226,235' : '255,197,212');
      const opacity = emphasized ? 0.42 + Math.random() * 0.36 : 0.34 + Math.random() * 0.52;
      const glow = emphasized ? (large ? 22 : 13) : (large ? 18 : 11);
      s.style.cssText = `position:absolute;border-radius:50%;background:rgba(${hue},0.92);
        ...
        opacity:${opacity};
        box-shadow:0 0 ${glow}px rgba(255,214,226,${emphasized ? 0.78 : 0.76});
