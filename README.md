...
**Dear Me; Dear You**는 사용자가 텍스트, 사진, 그림, 영상으로 편지를 작성하고 원하는 날짜에 이메일로 받아볼 수 있도록 만든 풀스택 웹 서비스입니다.
개인 편지함과 익명 열린 편지함을 분리해, 사적인 기록과 전시 환경에서의 공개 메시지를 각각 다른 경험으로 설계했습니다.

- 서비스 URL: https://dearme-dearyou.onrender.com
- Repository: https://github.com/jagseeun/DearMe-DearYou
...
| 역할 | 담당 |
| --- | --- |
| 기획 및 전체 구현, UI/UX 디자인, 배포 | 장세은 [jagseeun](https://github.com/jagseeun) |
| 아이디어 논의, QA | 이제희 [jeahee2](https://github.com/jeahee2) |

## Deployment Status

프론트엔드와 백엔드 모두 배포 및 연동을 완료했으며, 백엔드 서버는 운영 비용을 고려해 현재 상시 운영을 중단한 상태입니다.
시연 시에는 로컬 환경 또는 임시 배포 환경에서 실행할 수 있습니다.
...
루트 경로에 `.env` 파일을 생성합니다. 필요한 값은 `.env.example`을 참고합니다.
...
배포 환경에서는 다음 명령이 `postinstall`에서 실행됩니다.
